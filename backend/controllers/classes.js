...
exports.createClass = (req, res) => {
  ...
  gymClass
    ...
    .catch(error => {
      logger.error(
        `User ${pt} attempted to created a class but was not able too - error: ${error}`
      );
      res.status(500).json({
    ...
    err => {
      if (err) {
        ...
        GymClass.findOneAndUpdate(
          ...
          {
            $pull: { classMembers: { userId: userId } }
          },
          (err, doc) => {
            if (err) {
              logger.error(
                `User ${userId} was not able to be deleted from the ${classId} class.`
              );
              res.status(401).json({
                message: 'Error Occured!'
              });
              ...
              change.change(
                `User ${userId} was deleted from the ${classId} class.`
              );
              res.status(200).json({
                message: 'Success!'
              });
      ...
      if (err) {
        ...
        GymClass.findByIdAndUpdate(
          ...
          err => {
            if (err) {
              logger.error(
                `User ${userId} was not able to be booked to ${classId} class.`
              );
              res.status(401).json({
                message: 'Error Occured!'
              });
